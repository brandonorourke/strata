...
from strata_core.db import AsyncSessionLocal  # sync session for this script
...
def fetch_pending_articles(session, limit: int = 10) -> list[NewsArticle]:
    stmt = (
        select(NewsArticle)
...
def process_batch(limit: int = 10) -> int:
    with AsyncSessionLocal() as session:
        articles = fetch_pending_articles(session, limit=limit)
        if not articles:
            logger.info("No articles needing LLM enrichment.")
            return 0

        logger.info("Running LLM on %d articles", len(articles))
        updated = 0
...
def main():
    processed = process_batch(limit=5)
