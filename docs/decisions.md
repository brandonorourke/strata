...
## 2026-07-12 — Company universe is USASpending-bottom-up; equity data only annotates

- **Decision: build the trackable universe from USASpending (the FPDS ledger), not from an equity/ticker list.** Rank contractors by contract $, filter, and *annotate* tradeability afterward — don't start from "what's public."
- **Why USASpending is the spine:** it's the only source that (a) captures **everyone who competes for US contracts** — public, private, *and* foreign primes via their US subs (BAE Systems Inc., Thales US, Leonardo DRS all have UEIs); (b) carries **`recipient_uei` + the parent/children graph**, which makes the family rollup deterministic; (c) is the **complete record of FPDS actions**, down to the task/delivery orders that *are* the draw signal.
- **Equity data (ticker, tradeable-flag, market cap) is a thin ANNOTATION on the survivors, not a discovery step.** Run it on the few-hundred rolled-up companies above the size floor — not on 4,300 public names. It's a label, never a filter: privates and foreign primes stay in the graph, tagged `private` / `foreign-primary` / `US-ADR` / `US-listed`. Nobody is dropped for being untradeable — they're the competitive field.
- **Pipeline:** USASpending → roll UEI families up to companies → filter by size (family contract $) **and** sector (NAICS/PSC or DoD/USSF/intel agency) → annotate ticker + market cap → materiality (contract $ ÷ mktcap) → rank → **anchors** (tradeable × material, small/mid) + **context** (mega-primes, privates, foreign). Competitive field then auto-discovers from the anchors' vehicles. This is `apps/ingest/build_anchor_universe.py` (roadmap #5 made systematic).
- **Rejected — equity-first (ETF ∪ EDGAR-SIC → resolve down):** cleaner for *finding tradeable anchors*, but it only discovers competitors that are public/in-an-ETF, so it **misses the privates and foreign primes** that make the competitive field real. Wrong optimization for this product.
- **Rejected — SAM Opportunities CSV as the spine:** (a) **no UEI/DUNS for 2022+** (verified: 47-col archive, awardee = name string only) → no family rollup, no dedupe; (b) it's a **notice feed, not the award ledger** → misses task/delivery orders (the draws), sub-threshold, and non-noticed actions. Keep SAM CSV only as a *complement* (pre-award solicitations USASpending lacks; precise `PostedDate`; recent-window gap-fill). **If local/batch bulk is wanted, use the USASpending bulk download** (UEI + full FPDS) — same convenience, none of SAM's gaps.
- **The one filter USASpending can't do is public-vs-private/market-cap** — but that's the annotation layer, and it only ever *labels*, so the limitation doesn't affect universe completeness.
