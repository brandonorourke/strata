...
- **Primary-source pivot, ICFS first** (see `docs/decisions.md`, 2026-06-16): shift ingestion focus from news toward primary-source records. ICFS is the confirmed first build — a real customer wants it for his Viasat equity position, it's the most buildable, and no incumbent does FCC-for-investors monitoring. Full spec: `docs/specs/2026-06-23-icfs-v0.md`.
  - Then: broader FCC RSS (dockets, rulemakings, commissioner statements) — displaces a law-firm subscription the same customer pays for today. (Not yet spec'd.)
